...
sc = statsd.StatsClient(prefix="habhub.predictor")
...
def fresh(response_headers, request_headers):
    cc = pydap.open_url(response_headers)
    if "no-cache" in cc:
        return "STALE"
    return "FRESH"
...
def main():
    ...
    parser.add_option(
        ...
        dest="redirect",
        default="/dev/null",
        help="if forking, file to send stdout/stderr to",
        metavar="FILE",
    )
    parser.add_option(
        "-t",
    ...
    parser.add_option(
        ...
        action="count",
        dest="verbose",
        help="be verbose. The more times this is specified the more verbose.",
        default=False,
    )
    parser.add_option(
        "-p",
    ...
    if len(args) != 1:
        log.error(
            "Exactly one positional argument should be supplied (uuid). Defaulting to asdfasdfasdf"
        )
        # sc.incr("error")
        # sys.exit(1)
        uuid = "asdfasdfasdf"
    ...
    update_progress(
        gfs_percent=100, gfs_timeremaining="Done", gfs_complete=True, pred_running=True
    )
    ...
    command = [
        ...
        "-i" + gfs_dir,
        "-v",
        "-o" + uuid_path + "flight_path.csv",
        uuid_path + "scenario.ini",
    ] + alarm_flags
    ...
    while True:
        ...
        if "ERROR: Do not have wind data" in line:
            pred_output = [
                "One of the latitude, longitude or time deltas ({0}, {1}, {2}) was too small.".format(
                    options.latdelta, options.londelta, options.future
                ),
    ...
    if exit_code == 1:
        ...
        sc.incr("success_serious_warnings")
    elif pred_output:
        ...
        sc.incr("success_minor_warnings")
    else:
        assert exit_code == 0
...
def write_file(output_format, data, window, mintime, maxtime):
    ...
    longitudes = [
        x
        for x in enumerate(hgtprs_global.maps["lon"])
        if longitude_distance(x[1], window[2]) <= window[3]
    ]

    # Filter the latitudes we're actually going to use.
    latitudes = [
        x
        for x in enumerate(hgtprs_global.maps["lat"])
        if math.fabs(x[1] - window[0]) <= window[1]
    ]
    ...
    for timeidx, time in enumerate(hgtprs_global.maps["time"]):
        ...
        if (timestamp < datetime_to_posix(start_time)) | (
            timestamp > datetime_to_posix(end_time)
        ):
            continue
        ...
        for var in ("hgtprs", "ugrdprs", "vgrdprs"):
            ...
            if (longitudes[0][0] == 0) & (
                longitudes[-1][0] == hgtprs_global.maps["lat"].shape[0] - 1
            ):
                ...
                selection = grid[
                    timeidx, :, latitudes[0][0] : (latitudes[-1][0] + 1), :
                ]
            else:
                selection = grid[
                    timeidx,
        ...
        assert all(
            [
                x == target_shape
                for x in [x for x in iter(downloaded_data.values()) if x.shape]
            ]
        )
        ...
        log.debug("Using longitudes: %s" % ([x[1] for x in longitudes],))
        ...
        output.write(",".join([str(x[1]) for x in latitudes]) + "\n")
        output.write("# axis 3: longitudes\n")
        output.write(str(len(longitudes)) + "\n")
        output.write(",".join([str(x[1]) for x in longitudes]) + "\n")
            ...
            for latidx, latitude in enumerate(hgtprs.maps["lat"]):
                for lonidx, longitude in enumerate(hgtprs.maps["lon"]):
                    if longitude_distance(longitude, window[2]) > window[3]:
                        continue
                    record = (
                        hgtprs.array[pressureidx, latidx, lonidx],
...
def timestamp_to_datetime(timestamp):
    ...
    return datetime.datetime.fromordinal(ordinal_day) + datetime.timedelta(
        days=fractional_day
    )
...
def possible_urls(time, hd):
    ...
    if hd:
        url_format = "https://{host}/dods/gfs_hd/gfs_hd%i%02i%02i/gfs_hd_%02iz"
    else:
    ...
    while proposed >= earliest:
        for offset in (18, 12, 6, 0):
            possible_urls.append(
                url_format % (proposed.year, proposed.month, proposed.day, offset)
            )
...
if __name__ == "__main__":
    try:
        ...
        if e.code != 0 and progress_f:
            ...
            sc.incr("unknown_error_exit")
        raise
    except Exception as e:
        sc.incr("uncaught_exception")
